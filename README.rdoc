...
dagnabit is only known to work with SQLite, but should work on any SQL database
that supports transaction-scoped temporary tables and subselects.  It does not
(yet) work on Oracle XE 10g databases via the ActiveRecord oracle-enhanced
adapter.
