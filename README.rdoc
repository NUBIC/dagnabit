...
Details for creating the edge tables are discussed below.
...
Both +acts_as_dag_node+ and +acts_as_dag_edge+ accept configuration options.
These configuration options are discussed below.

=== Edge table schemas

Edge storage is accomplished with two tables having identical schemas.  One
table holds explicitly specified edges between nodes.  The other table contains
the _transitive closure_ of all graphs, i.e. one tuple per path in the graph.
(The transitive closure table is therefore a superset of the direct edge
table.)
...
=== Configuration options for +acts_as_dag_edge+

* +transitive_closure_table_name+: The table where tuples in the transitive
  closure of the graph should be stored.  Defaults to
  <tt>#{edge_table_name}_transitive_closure_tuples</tt>.
* +descendant_id_column+: Column in the edge tables for recording the ID of a
  descendant.  Defaults to +descendant_id+.
* +ancestor_id_column+: Column in the edge tables for recording the ID of a
  ancestor.  Defaults to +ancestor_id+.
* +descendant_type_column+: Column in the edge tables for recording the type of a
  descendant.  Defaults to +descendant_type+.
* +ancestor_type_column+: Column in the edge tables for recording the type of a
